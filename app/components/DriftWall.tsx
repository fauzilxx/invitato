...
export interface DriftWallProps {
  items?: DriftWallItem[] | DriftWallItem[][];
  columns?: number;
  tileWidth?: number;
  tileHeight?: number;
  gap?: number;
...
const DriftWall = ({
  ...
  const columnItems = useMemo<DriftWallItem[][]>(() => {
    if (Array.isArray(items[0])) {
      return items as DriftWallItem[][];
    }
    const flatItems = items as DriftWallItem[];
    const cols: DriftWallItem[][] = Array.from({ length: columns }, () => []);
    flatItems.forEach((item, i) => cols[i % columns].push(item));
    return cols.map((col) => (col.length ? col : flatItems.slice(0, 1)));
  }, [items, columns]);
  ...
  const columnMeta = useMemo<ColumnMeta[]>(() => {
    ...
    return columnItems.map((col) => {
      ...
      const copies = Math.max(5, Math.ceil((containerHeight * 3.5) / copyHeight) + 3);
  ...
  const baseVelocities = useMemo<number[]>(() => {
    ...
    return columnItems.map((_, c) => {
      ...
      const factor = Math.max(0.65, columnFactor(c, variance));
      return speed * factor * dirSign * altSign;
    });
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Drifting wall of pre-wedding tiles"
    >
